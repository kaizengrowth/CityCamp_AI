...
class NotificationService:
    ...
    async def send_meeting_notification(
        self, 
        meeting: Meeting, 
        notification_type: str = "meeting_alert"
    ) -> int:
        """
        ...
        try:
            ...
            users = self.db.query(User).filter(
                User.notification_preferences.op('->>')('meetings') == 'true'
            ).all()
            ...
            self.db.commit()
            logger.info(f"Created {notifications_created} meeting notifications for meeting {meeting.id}")
    ...
    def _create_meeting_message(self, meeting: Meeting) -> str:
        ...
        message_parts.append("Log in to CityCamp AI to view more details and set your preferences.")
    ...
    async def cleanup_old_notifications(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            deleted_count = self.db.query(Notification).filter(
                ...
                Notification.opened == True  # Only delete opened notifications
            ).delete()
