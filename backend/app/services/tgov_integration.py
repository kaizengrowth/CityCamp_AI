...
class TGOVIntegrationService:
    ...
    def __init__(self, db: Session):
        ...
        self.tgov_api_base = (
            "https://api.tgov-scraper.codefortulsa.org"  # Hypothetical API
        )
            ...
            mock_tgov_data = [
                {
                    ...
                    "video_url": (
                        ...
                        "mp4:archive/tulsa-ok/tulsa-ok_843d30f2.mp4/playlist.m3u8"
                    ),
                    "transcription_url": (
                        "https://data.tgov-scraper.org/transcriptions/2024-01-15.txt"
                    ),
                    "subtitled_video_url": (
                        "https://data.tgov-scraper.org/videos/2024-01-15-subtitled.html"
                    ),
                    "translated_transcriptions": {
                        "es": (
                            "https://data.tgov-scraper.org/transcriptions/2024-01-15-es.txt"
                        ),
                        "mya": (
                            "https://data.tgov-scraper.org/transcriptions/2024-01-15-mya.txt"
                        ),
                    },
                    "agenda_items": [
                        {
                        ...
                        },
                        {
                            ...
                            "description": (
                                "Commercial zoning variance for downtown development"
                            ),
    ...
    async def sync_meeting_with_tgov_data(self, tgov_meeting: Dict) -> Meeting:
        ...
        existing_meeting = (
            ...
            .filter(Meeting.external_id == tgov_meeting["meeting_id"])
            .first()
        )

        # Parse the meeting date
        date_str = tgov_meeting["date"].replace("Z", "+00:00")
        ...
        if existing_meeting:
            ...
            if tgov_meeting.get("transcription_url"):
                ...
                existing_meeting.summary = (
                    f"Transcription available: {tgov_meeting['transcription_url']}"
                )
            ...
            meeting = Meeting(
                ...
                source="tgov_scraper",
                summary=(
                    f"Transcription available: "
                    f"{tgov_meeting.get('transcription_url', 'N/A')}"
                ),
            )
            self.db.add(meeting)
    ...
    async def get_meeting_transcription(self, meeting_id: str) -> Optional[str]:
        ...
        meeting = (
            self.db.query(Meeting).filter(Meeting.external_id == meeting_id).first()
        )

        if (
            meeting
            and meeting.summary
            and "Transcription available:" in meeting.summary
        ):
