...
class MeetingService:
    def __init__(self, db: Session):
        self.db = db
        self.base_url = (
            "https://api.tulsacouncil.org"  # Update with actual API endpoint
        )
        ...
        try:
            ...
            mock_meetings = [
                ...
                },
                {
                    ...
                    "agenda_url": (
                        "https://www.cityoftulsa.org/planning-agenda-2024-01-18.pdf"
                    ),
    ...
    async def create_or_update_meeting(self, meeting_data: dict) -> Meeting:
        ...
        existing_meeting = (
            ...
            .filter(Meeting.external_id == meeting_data["id"])
            .first()
        )
        ...
        if existing_meeting:
            ...
            # Create new meeting
            meeting_date_str = meeting_data["meeting_date"].replace("Z", "+00:00")
            meeting_data["meeting_date"] = datetime.fromisoformat(meeting_date_str)
            meeting = Meeting(
    ...
    async def get_meetings_by_date_range(
        ...
        return (
            self.db.query(Meeting)
            .filter(
                Meeting.meeting_date >= start_date, Meeting.meeting_date <= end_date
            )
            .all()
        )
    ...
    async def get_meetings_by_type(self, meeting_type: str) -> List[Meeting]:
        ...
        return self.db.query(Meeting).filter(Meeting.meeting_type == meeting_type).all()

    async def search_meetings_by_topic(self, topic: str) -> List[Meeting]:
        """Search meetings by topic/keyword"""
        return self.db.query(Meeting).filter(Meeting.topics.contains([topic])).all()

    async def get_user_relevant_meetings(
