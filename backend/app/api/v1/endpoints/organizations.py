...
def list_organizations(
    ...
    search: Optional[str] = Query(None, description="Search in organization name and description"),
...
def get_organization_types(db: Session = Depends(get_db)):
    ...
    """
    types = db.query(Organization.organization_type).distinct().filter(
        Organization.organization_type.isnot(None),
        Organization.is_active == True
    ).all()
...
def get_focus_areas(db: Session = Depends(get_db)):
    ...
    organizations = db.query(Organization.focus_areas).filter(
        Organization.focus_areas.isnot(None),
        Organization.is_active == True
    ).all()
...
def get_organization_stats(db: Session = Depends(get_db)):
    ...
    verified_orgs = db.query(Organization).filter(
        and_(Organization.is_active == True, Organization.is_verified == True)
    ).count()
