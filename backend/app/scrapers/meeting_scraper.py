...
class MeetingScraper:
    ...
    async def _is_new_meeting(self, meeting: Meeting) -> bool:
        ...
        if meeting.created_at and meeting.created_at > datetime.now() - timedelta(hours=1):
            return True
        return False
        ...
        try:
            ...
            for user in interested_users:
                await self.notification_service.create_meeting_notification(
                    ...
                    message=f"A new meeting has been scheduled for {meeting.meeting_date.strftime('%B %d, %Y at %I:%M %p')}",
                    send_email=user.email_notifications,
                    send_sms=user.sms_notifications
                )
            
        except Exception as e:
            logger.error(f"Error sending notifications for meeting {meeting.id}: {str(e)}")
    ...
    async def get_scraping_stats(self) -> Dict[str, int]:
        ...
        meetings_with_minutes = self.db.query(Meeting).filter(
        ...
        return {
            ...
            'scraping_coverage': (meetings_with_minutes / total_meetings * 100) if total_meetings > 0 else 0
        }
